...
于是有了 `FGO_APPLE_AP_MIN`（默认 120）：AP 不到阈值就跳过本次合成，只打印一行"未达合成阈值"。配合 workflow 里**每 3 小时**的静态调度——每轮合成花 40 AP、3 小时只回复 36 AP，会自动收敛到"合成几轮、歇一轮"的节奏，稳态约 7 个苹果/天（288 AP/天回复上限封顶）；玩家把 AP 花掉了，最迟 3 小时后下次运行自动回归正常节奏。比动态改写 cron 干净：没有提交噪音，调度自纠错。
